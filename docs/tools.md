...
All 28 tools, in the order an agent typically meets them. Every agent session
...
| Tool | What it does |
|---|---|
| `statamic_overview` | Call this first. Sites, each with `can_access`, whether you may work in it; the collections, taxonomies, global sets, asset containers, navigations, and forms exposed to MCP and visible to you (forms carry `stores_submissions`, false on a form that only sends email, and their submission count); your capability flags per resource (`can_create`, `can_edit`, `can_publish`, `can_upload`, `can_delete` — delete flags appear only when deletes are enabled; collections whose blueprint has an `author` field add `can_edit_other_authors`, `can_publish_other_authors`, and `can_delete_other_authors`); `date_behavior` on dated collections (`future` and `past`: `public`, `unlisted`, or `private`, as in the collection settings); on multisite, each collection's `sites`, `propagate` (whether `entries_create` makes a localization in every site at once), and `origin_behavior` (`root`: `entries_localize` always localizes from the root entry); the acting user, including their `id`; the server block (`read_only`, `deletes`, and `timezone`, the zone a date without an offset is read in); the site's `guidelines` when Tools → MCP → Guidelines has a site text. |
| `blueprints_get` | A blueprint's fields (handle, type, rules, required, options, instructions, `time_enabled` on date fields, and on multisite `localizable`: only such a field can hold a value of its own in a localization) plus a valid example payload for writes. Works for collections, taxonomies, globals, and forms, where the fields are the keys of a submission's data. On collection and taxonomy blueprints, `slug` (and `date` on dated collections) is listed in the fields but left out of the example, because the entry and term write tools take it as a top-level parameter; `example_notes` says so. Replicator and Bard fields list their `sets`, the blocks of a page builder, each with its display name, group, and instructions. Pass `set` with a set's handle to get that set's fields and an example row. A handle is found anywhere in the blueprint, sets inside other sets included; when it has different fields in different places, the error lists their paths, such as `page_builder.section_combined.sections.section_hero`, to pass instead. Grid and group fields list their fields, and asset, entry, term, and user fields report `container`, `collections`, `taxonomies`, `max_files`, and `max_items` when configured. `example_notes` keys notes on nested values by their path in the example. Tabs and sections with instructions come back in `tabs`, with the handles of the fields under them. The Guidelines page's rows for the collection or taxonomy come back as `guidelines`. See [guidelines.md](guidelines.md). |
...
| `entries_get` | Full entry by id or collection + slug. A slug that several entries of the site share, as pages under different parents can, is an error that lists their ids and URLs. Raw (round-trippable) by default; `format=augmented` for display only. Long rich-text values are truncated to previews unless requested via `fields`. On revision-enabled entries, `has_working_copy` reports staged changes. The returned data is the live entry unless you pass `working_copy: true`, which returns the staged working copy, the version `entries_publish` would promote; `source` says which one you got. `blueprint` names the entry's blueprint, the one `blueprints_get` describes. |
| `entries_create` | Raw-data create through the same validate-and-process steps as the CP's save, so stored values match what the CP writes. Always saves an unpublished **draft**; nothing goes live here. On revision-enabled collections the draft gets an initial revision attributed to you. On a structured collection the entry goes into the collection's tree, at the top level or under `parent`, the id of another entry in the same collection and site. Like the CP, it refuses a URL another entry of the site already has. On a collection in more than one site the entry exists in that site only, unless the collection's `propagate` is on and every site gets a localization at once; the response lists them under `localizations`. |
| `entries_localize` | Only registered on multisite installs (`statamic.system.multisite`). Adds an entry to another site of its collection, the way the CP's Localize action does. The new entry is a **draft** localization of the one passed: it inherits every field it does not override, `data` holds its own values (a translated title and content), and `slug` defaults to the origin's. Only fields the blueprint marks localizable take a value of their own; the rest show the origin's value in every site, and the CP makes them read-only there. The origin is the entry passed, or the root entry when the collection's `origin_behavior` is `root`, as in the CP. On a structured collection it joins the target site's tree under the localization of the origin's parent, or at the top level when the parent has none there. Needs the edit permission for the origin entry, as in the CP, plus access to the target site. A site that already has the entry is an error naming that entry. |
| `entries_update` | Shallow top-level merge of raw data (nested structures replaced wholesale). Never changes publish state. On a localization, `data` may only name fields the blueprint marks localizable; the rest are changed on the origin entry. `site` is a selector and never moves an entry: `entries_localize` adds one to another site. On revision-enabled collections, edits to a published entry become a **working copy** — the live entry is never touched; an existing working copy is amended (created vs amended is stated in the result). Without revisions, edits save straight to the entry, so changes to a published entry are live at once. No-op updates save nothing. `parent` moves the entry and its children in a structured collection's tree. The id of an entry in the same collection and site makes it that entry's last child, and `""` moves it to the top level. An omitted or null `parent` leaves the entry where it is. A move needs the collection's reorder permission on top of edit, so on a blueprint with an `author` field, moving someone else's entry also needs `edit other authors`. The move saves the live tree at once, also on revision-enabled collections, because working copies don't store an entry's place in the tree. An update that would give the entry a URL another entry already has is refused, as in the CP. |
| `entries_preview` | A short-lived URL that renders the entry through the site's own templates, drafts included, using Statamic's Live Preview. On revision-enabled entries it shows the staged working copy, not the live entry. Needs the collection's edit permission, or `edit other authors` for an entry you are not an author of on a blueprint with an `author` field, the same check as Live Preview in the CP. Anyone with the URL can open it until `expires_at`, an hour after the call. `target` picks one of the collection's preview targets, and the response lists them all in `targets`. An entry in a collection without a route has no page, so the call fails. `read_only` keeps this tool because it changes no content. The page shows the entry as it was when you called the tool, so call it again after an edit. |
...
On a blueprint with an `author` field, the write tools and `entries_preview` also apply
Statamic's author rules: an entry you are not an author of needs the "other authors"
permission, and `entries_create` makes you the author unless `data` names one. See
[How authorization works](permissions.md#how-authorization-works).

On a multi-site install an entry belongs to one site. `entries_get` returns
`localizations`, one key per site of the collection you can access, with the id and
status of the entry there, or null where it has none. To translate a page, call
`entries_localize` with the origin's id, the target site, and the translated fields
in `data`, then `entries_update` on the new entry for further changes and
`entries_publish` to take it live in that site. A field the blueprint doesn't mark
localizable follows the origin in every site, in the CP as well, so mark the fields
to translate as localizable in the blueprint; `blueprints_get` reports the flag.
Collections with `propagate` on get a localization in every site from
`entries_create` already; `statamic_overview` reports each collection's `sites`,
`propagate`, and `origin_behavior`.
...
| Tool | What it does |
|---|---|
| `submissions_list` | A form's submissions, newest first, each with its id, date, and full data keyed by field handle. Submissions are flat and small, so the list carries the data itself instead of sending you back for each one. `search` is the CP's search: a substring match over the text, textarea, and integer fields, where `%` and `_` are wildcards, and over the date as stored (`YYYY-MM-DD HH:MM:SS`, UTC). Filter by date with `since` (inclusive) and `before` (exclusive) instead. A field the visitor did not send is absent from `data`. A time without an offset is read in `server.timezone`, and a date alone means midnight, so `before: 2026-09-27` covers everything up to the end of the 26th. Paginated like the other list tools. Submissions are not per site. |
| `submissions_get` | One submission by form handle and id, with `cp_url`. The id is looked up within that form only. |
| `submissions_delete` | Only registered when `deletes` is enabled. Removes the submission. Files it uploaded stay in their asset container, as when deleting in the CP. |

Reading takes `view {form} form submissions` and deleting takes `delete {form} form submissions`. `configure forms` grants both on every form, which is how Statamic's own form policies read it. Nothing creates or edits a submission, because the Control Panel can't either: a submission is what a visitor sent through the front-end form, with its honeypot, validation, and emails. `statamic_overview` lists the forms you may read with `stores_submissions` (false on a form that only sends email) and their submission count, and `blueprints_get` with `type: form` names the fields.
