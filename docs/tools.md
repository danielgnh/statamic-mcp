...
| `entries_update` | Shallow top-level merge of raw data (nested structures replaced wholesale). Never changes publish state. On revision-enabled collections, edits to a published entry become a **working copy** — the live entry is never touched; an existing working copy is amended (created vs amended is stated in the result). Without revisions, edits save straight to the entry, so changes to a published entry are live at once. No-op updates save nothing. `parent` moves the entry and its children in a structured collection's tree. The id of an entry in the same collection and site makes it that entry's last child, and `""` moves it to the top level. An omitted or null `parent` leaves the entry where it is. A move needs the collection's reorder permission and saves the live tree at once, also on revision-enabled collections, because working copies don't store an entry's place in the tree. |
