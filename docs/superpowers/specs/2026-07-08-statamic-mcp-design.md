...
No database tables, no CP UI, no views, no JS. ~20 classes.
...
Names are explicit via `#[Name]` (snake_case, resource-first so related tools sort together). All list tools paginate (default `per_page` from config, hard cap 100 in code) returning totals + next-page hints. All content tools accept optional `site` (defaults to `Site::default()->handle()`); single-site installs never see it.

| # | Tool | Annotations | Contract |
|---|---|---|---|
| 1 | `statamic_overview` | read-only, idempotent | Zero params. Returns: sites; exposed collections (handle, title, dated?, revisions?, blueprints); taxonomies; global sets; **acting user** (email, roles, is_super) + effective capability flags; server flags (`read_only`, deletes enabled). Filtered to the config allowlist AND what the user may view. |
| 2 | `blueprints_get` | read-only | `type` (collection\|taxonomy\|global), `handle`, optional `blueprint`. Returns fields via the **Fields API** (handle, type, rules, required, options, instructions) — never YAML parsing (v6 tabs-vs-sections YAML shape is unverified) — plus a complete valid example payload (placeholder relation/asset IDs obviously fake, e.g. `"REPLACE-WITH-REAL-ENTRY-ID"`). |
...
| 6 | `entries_update` | write, idempotent | `id`, `data` merge-patch, optional `slug`, `date`, `site` (selector of an existing localization only — never creates/moves), `published`. Publish state untouched unless `published` explicitly sent. Explicit `null` clears a field (with intelligible error if that violates required). |
| 7 | `entries_delete` | destructive | `id`. Registered only when deletes enabled (see §6). |
| 8–12 | `terms_list/get/create/update/delete` | as entries | Same contracts against `Term` facade; `taxonomy` instead of `collection`; no `date`/`published` (terms have no status), localization per Statamic term rules. |
| 13 | `globals_get` | read-only | `handle` (or omit → all exposed sets), optional `site`. Returns variables for the site localization. |
...
- Collections with **revisions enabled**: data writes create an unpublished **working copy** through the same revision mechanism the CP uses (attributed to the acting user, with a revision message naming the tool) — never mutating the live entry; `published: true` on such collections is rejected with "this collection uses revisions; publish from the Control Panel". *(Exact working-copy API call flagged for build-time verification — the behavioral contract is fixed regardless.)*
...
- **Raw data is the only write shape.** Writes validate through Statamic's own pipeline — `$blueprint->fields()->addValues($merged)->validator()->validate()` where `$merged = array_merge($existingValues, $patch)` — so partial updates never false-fail required fields. Validation failures → `Response::error()` with field-level messages (never protocol errors; enables one-round-trip self-correction).
